...
@endsection
@section('enlaces')
    <a href="{{route('parametros.index')}}" class="font-bold text-gray-600">Parametros /</a>  Marcas
@endsection
@section('contenido')
<div class="px-5 mb-16">
    <div class="flex justify-end mb-5">
        <a href="{{route('brands.create')}}" class="px-4 py-2 bg-green-500 rounded-lg text-white shadow hover:bg-green-600 hover:shadow-lg">
            ...
            @if (count($brands) != 0)
                @foreach ($brands as $brand)
                    <tr class="text-gray-500 text-sm">
                        <td class="border border-slate-300 p-2 ">{{$brand->id}}</td>
                        <td class="border border-slate-300 p-2">{{$brand->name}}</td>
                        <td class="border border-slate-300 p-2">{{$brand->description}}</td>
                        <td class="border border-slate-300 p-2">{{$brand->statu->name}}</td>
                        <td class="border border-slate-300 flex p-2">
                            <a href="{{route('brands.show',$brand->id)}}" class="bg-blue-500 px-3 py-2 rounded text-white mr-3">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
