                        ...
                        <tr class="text-gray-500 text-sm">
                            ...
                            <td class="border border-slate-300 p-2">
                                ...
                                @if (!is_null($processor->computer_id))
                                    Asignado: {{$processor->computer->serial}}  <br>
                                    Maquina: Ordenador 
                                @endif                           
                                @if (is_null($processor->notebook_id) && is_null($processor->computer_id))
                                    {{"Sin asignar"}}    
                                @endif    
                            </td>
                            <td class="border border-slate-300 flex p-2">
                                <a href="{{route('processors.show',$processor->id)}}" class="bg-blue-500 px-3 py-2 rounded text-white mr-3">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
