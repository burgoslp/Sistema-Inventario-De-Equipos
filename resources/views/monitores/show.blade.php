@extends('layouts.app')
@section('titulo')
   Componentes / Monitores / Actualizar 
@endsection
@section('enlaces')
    <a href="{{route('componentes.index')}}" class="font-bold text-gray-600">Componentes /</a>  <a href="{{route('monitores.index')}}" class="font-bold text-gray-600">Monitores</a> / Actualizar
@endsection
@section('contenido')
    <div class="px-5 ">
        <form action="{{route('monitores.update')}}" method="POST" enctype="multipart/form-data">   
            @method('PUT')      
            @csrf   
            <input type="hidden" name="monitor_id" value="{{$monitor->id}}">
            <div class="grid  bg-white shadow-lg p-5">
                <div class="flex justify-center">
                    <img src="{{asset('img/monitores/'.$monitor->image)}}" alt="imagen del monitor real" class="rounded-full w-1/4">
                </div>
                <div class="mb-5">
                    <label class="block uppercase tracking-wide text-gray-700 text-xs font-bold mb-2" for="grid-first-name">
                        Serial:
                    </label>
                    <input class="appearance-none block w-full bg-gray-200 text-gray-700 border @error('serial') border-red-500 @enderror rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white" id="serial" name="serial" type="text" placeholder="Ingrese serial" value="{{$monitor->serial}}">
                    @error('serial')
                ...
                <div  class="mb-5">
                    ...
                    <input class="appearance-none block w-full bg-gray-200 text-gray-700 border @error('observation') border-red-500 @enderror rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white" id="observation" name="observation" type="text" placeholder="Ingrese observación" value="{{$monitor->observation}}">
                    @error('observation')
                ...
                <div  class="mb-5">
                    ...
                    <div class="relative">
                        <select class="block appearance-none w-full bg-gray-200 border border-gray-200 text-gray-700 py-3 px-4 pr-8 rounded leading-tight focus:outline-none focus:bg-white focus:border-gray-500" id="monitor_size_id" name="monitor_size_id">
                            ...
                            @if ($monitor->monitor_size_id == $tamaño->id)
                                <option value="{{$tamaño->id}}" selected>{{$tamaño->cantidad." ".$tamaño->unidad}}</option>
                            @else
                    ...
                    <div class="relative">
                        <select class="block appearance-none w-full bg-gray-200 border border-gray-200 text-gray-700 py-3 px-4 pr-8 rounded leading-tight focus:outline-none focus:bg-white focus:border-gray-500" id="monitor_frame_id" name="monitor_frame_id">
                            @foreach ($frecuencias as $frecuencia)
                                @if ($monitor->monitor_frame_id == $frecuencia->id)
                                    <option value="{{$frecuencia->id}}" selected>{{$frecuencia->cantidad." ".$frecuencia->unidad}}</option>
                                @else
                    ...
                    <div class="relative">
                        <select class="block appearance-none w-full bg-gray-200 border border-gray-200 text-gray-700 py-3 px-4 pr-8 rounded leading-tight focus:outline-none focus:bg-white focus:border-gray-500" id="monitor_resolution_id" name="monitor_resolution_id">
                            @foreach ($resoluciones as $resolucion)
                                @if ($monitor->monitor_resolution_id == $resolucion->id)
                                    <option value="{{$resolucion->id}}" selected>{{$resolucion->name}}</option>
                                @else
                    ...
                    <div class="relative">
                        <select class="block appearance-none w-full bg-gray-200 border border-gray-200 text-gray-700 py-3 px-4 pr-8 rounded leading-tight focus:outline-none focus:bg-white focus:border-gray-500" id="connector_id" name="connector_id">
                            @foreach ($conectores as $conector)
                                @if($monitor->connector_id == $conector->id)
