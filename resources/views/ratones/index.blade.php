...
@endsection
@section('enlaces')
    <a href="{{route('componentes.index')}}" class="font-bold text-gray-600">Componentes /</a>  Ratones
@endsection
@section('contenido')
<div class="px-5">
    <div class="flex justify-end mb-5">
        <a href="{{route('ratones.create')}}" class="px-4 py-2 bg-green-500 rounded-lg text-white shadow hover:bg-green-600 hover:shadow-lg">
    ...
    <div class="bg-white shadow-lg">
        <table class="table-auto text-left border-collapse border border-slate-400 w-full">
            ...
            <tbody>
                    ...
                    @foreach ($ratones as $raton)
                        <tr class="text-gray-500 text-sm">
                            ...
                            <td class="border border-slate-300 p-2">
                                ...
                                Marca: {{$raton->brand->name}} <br>
                                Conexión: {{$raton->connection->name}}<br>
                                Conector: {{$raton->connector->name}}<br>                                
                            </td>
                            <td class="border border-slate-300 p-2">{{$raton->statu->name}}</td>
                            <td class="border border-slate-300 flex p-2">
                                <a href="{{route('ratones.show',$raton->id)}}" class="bg-blue-500 px-3 py-2 rounded text-white mr-3">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" />
                                    </svg> 
                                </a>                         
                                <form action="{{route('ratones.delete')}}" method="POST" class="mr-3">
                                    @method('DELETE')
                                    @csrf
                                    <input type="hidden" value="{{$raton->id}}" name="raton_id">
                ...
                @else
                    <tr class="text-gray-500 text-sm">
                        <td colspan="5" class="border border-slate-300 p-2 text-center text-xl"> No se han registrado ratones</td>                    
                    </tr>  
                @endif
            </tbody>
          </table>
