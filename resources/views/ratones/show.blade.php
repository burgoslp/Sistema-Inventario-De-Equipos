@extends('layouts.app')
@section('titulo')
   Componentes / Ratones / Actualizar 
@endsection
@section('enlaces')
    <a href="{{route('componentes.index')}}" class="font-bold text-gray-600">Componentes /</a>  <a href="{{route('ratones.index')}}" class="font-bold text-gray-600">Ratones</a> / Actualizar
@endsection
@section('contenido')
    <div class="px-5 ">
        <form action="{{route('ratones.update')}}" method="POST" enctype="multipart/form-data">   
            @method('PUT')      
            @csrf   
            <input type="hidden" name="mouse_id" value="{{$raton->id}}">
            <div class="grid  bg-white shadow-lg p-5">
                <div class="flex justify-center">
                    <img src="{{asset('img/ratones/'.$raton->image)}}" alt="imagen del monitor real" class="rounded-full w-1/4">
                </div>
                <div class="mb-5">
                    <label class="block uppercase tracking-wide text-gray-700 text-xs font-bold mb-2" for="grid-first-name">
                        Serial:
                    </label>
                    <input class="appearance-none block w-full bg-gray-200 text-gray-700 border @error('serial') border-red-500 @enderror rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white" id="serial" name="serial" type="text" placeholder="Ingrese serial" value="{{$raton->serial}}">
                    @error('serial')
                ...
                <div  class="mb-5">
                    ...
                    <input class="appearance-none block w-full bg-gray-200 text-gray-700 border @error('observation') border-red-500 @enderror rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white" id="observation" name="observation" type="text" placeholder="Ingrese observación" value="{{$raton->observation}}">
                    @error('observation')
                    ...
                    <div class="relative">
                        <select class="block appearance-none w-full bg-gray-200 border border-gray-200 text-gray-700 py-3 px-4 pr-8 rounded leading-tight focus:outline-none focus:bg-white focus:border-gray-500" id="connection_id" name="connection_id">
                            @foreach ($conexiones as $conexion)
                                @if($raton->connection_id == $conexion->id)
                                    <option value="{{$conexion->id}}" selected>{{$conexion->name}}</option>
                                @else 
                    ...
                    <div class="relative">
                        <select class="block appearance-none w-full bg-gray-200 border border-gray-200 text-gray-700 py-3 px-4 pr-8 rounded leading-tight focus:outline-none focus:bg-white focus:border-gray-500" id="connector_id" name="connector_id">
                            @foreach ($conectores as $conector)
                                @if($raton->connector_id == $conector->id)
