...
@endsection
@section('enlaces')
    <a href="{{route('componentes.index')}}" class="font-bold text-gray-600">Componentes /</a>  Rams
@endsection
@section('contenido')
<div class="px-5">
    <div class="flex justify-end mb-5">
        <a href="{{route('rams.create')}}" class="px-4 py-2 bg-green-500 rounded-lg text-white shadow hover:bg-green-600 hover:shadow-lg">
    ...
    <div class="bg-white shadow-lg">
        <table class="table-auto text-left border-collapse border border-slate-400 w-full">
            ...
            <tbody>
                @if (count($rams) != 0)    
                    @foreach ($rams as $ram)
                        <tr class="text-gray-500 text-sm">
                            ...
                            <td class="border border-slate-300 p-2">{{$ram->serial}}</td>
                            <td class="border border-slate-300 p-2">
                                Observación: {{$ram->observation}} <br>
                                Marca: {{$ram->brand->name}} <br>
                                capacidad: {{$ram->capacidad}} {{$ram->unidad}}<br>
                                Frecuencia: {{$ram->frecuencia}} <br>
                            ...
                            <td class="border border-slate-300 flex p-2">
                                ...
                                <form action="{{route('rams.delete')}}" method="POST" class="mr-3">
                                    @method('DELETE')
                                    @csrf
                ...
                @else
                    <tr class="text-gray-500 text-sm">
                        <td colspan="5" class="border border-slate-300 p-2 text-center text-xl"> No se han registrado rams</td>                    
                    </tr>  
                @endif
            </tbody>
          </table>
