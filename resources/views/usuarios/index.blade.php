...
<div class="px-5">
    <div class="flex justify-end mb-5">
        <a href="{{route('usuarios.create')}}" class="px-4 py-2 bg-green-500 rounded-lg text-white shadow hover:bg-green-600 hover:shadow-lg">
                ...
                @if (count($users) != 0)
                    @foreach ($users as $user)
                        <tr class="text-gray-500 text-sm">
                            ...
                            <td class="border border-slate-300 p-2">{{$user->username}}</td>
                            <td class="border border-slate-300 p-2">{{$user->email}}</td>
                            <td class="border border-slate-300 p-2">{{$user->statu->name}}</td>
                            <td class="border border-slate-300 flex p-2">
                                <a href="{{route('usuarios.show',$user->id)}}" class="bg-blue-500 px-3 py-2 rounded text-white mr-3">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
